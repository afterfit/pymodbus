...
class ModbusProtocol(asyncio.BaseProtocol):
    ...
    def __close(self, reconnect: bool = False) -> None:
        ...
        if not reconnect and self.reconnect_task:
            ...
            # pattern2 = KHONG tu phuc hoi (branch debug, KHONG MERGE). Tai lan cancel dau tien cua
            # client SERIAL, thay vi cancel R1 (task do_reconnect dang chay khi close nay xay ra)
            # thi bat _pattern2_arm theo comm_name. Task do_reconnect nao mo duoc port cho client do
            # se tu huy giua chung va bo lai mot phantom giu port (xem create_serial_connection);
            ...
            if self.comm_params.comm_type == CommType.SERIAL and self._cancel_n == 1:
                ...
                _st._pattern2_arm[0] = self.comm_params.comm_name
                Log.debug(
                    "TASKDBG PATTERN2 cancel#{} comm={} -> bat _pattern2_arm (phantom tu huy khi mo port)",
